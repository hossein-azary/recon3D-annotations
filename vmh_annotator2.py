...
class VMHAnnotator:
    ...
    def _normalize_sbo_annotation(annotation: Dict) -> None:
        """Use lowercase 'sbo', merge uppercase 'SBO', strip prefix, and deduplicate."""
        sbo_values = []
        for key in ("sbo", "SBO"):
    ...
    def _normalize_prefixed_identifiers(annotation: Dict) -> None:
        ...
        for key in ("ec-code", "sbo", "chebi", "inchi"):
...
def main() -> None:
  ...
  python vmh_annotator2.py Recon3D_annotated.json --report report.html --report-only
...
""",
    ...
    parser.add_argument(
        "--report-only",
        action="store_true",
        help="Generate an HTML report from the input model without calling VMH or saving a model",
    )
